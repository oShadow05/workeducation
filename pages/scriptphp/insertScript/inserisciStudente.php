...
$nFieldsValue = json_decode(stripslashes($_POST['nValuesFields']));

$stValues = json_decode(stripslashes($_POST['values']));

$bindValues = str_repeat('?,', $nFieldsValue - 1) . '?';


$insert = $query->insert(addslashes($_SESSION["tableStudente"]), $bindValues, $stValues);


if($insert==false)
{
  echo "<h4 id='er-cred'>Errore nell'inserimento dello studente</h4>";
}
else
{
  echo "<h4>Studente inserito!</h4>";
}
